      ...
      <div className="container-page max-w-2xl">
        ...
        <div className="mt-8 grid gap-3 sm:grid-cols-2">
          {[
            ...
            <Link
              key={href}
              href={href}
              className="rounded-card border border-rule bg-white p-4 font-semibold text-blue-700 hover:border-blue-600"
            >
              {label}
            </Link>
          ))}
        ...
        <details className="mt-8">
          ...
          <ul className="mt-3 grid gap-1.5 sm:grid-cols-2">
            {services.map((s) => (
              <li key={s.slug}>
                <Link href={`/${s.slug}/`} className="text-blue-700 hover:underline">
                  {s.navLabel}
                </Link>
              </li>
            ))}
